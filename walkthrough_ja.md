...
### 👮‍♂️ 管理者用有給モニタリング (2026-02-08)
- **モニタリング画面**: 管理者ダッシュボードに「有給モニタリング」タブを追加。全職員の取得義務進捗を一覧表示。
- **アラート機能**:
    - **義務未達**: 5日未満の職員をオレンジ色で警告。
    - **期限切迫**: 期限まで3ヶ月を切り、かつ未達の職員を赤色で強調表示。
    - **義務違反**: 直前の1年間（過去期間）で5日未達だった場合、赤色の「義務違反」バッジが表示されます。
- **基準日について**: 入社日の6ヶ月後を「初回基準日」とし、それ以降1年ごとに基準日を更新します。
- **クイックフィルタ**: 「未達・要注意のみ」ボタンで、フォローが必要な職員を即座に抽出可能。

### 🇯🇵 エラーメッセージの完全日本語化 (2026-02-08)
- **バックエンド**: `ValidationMessages.properties` を導入し、入力値チェック（必須、文字数など）のエラーメッセージをシステムレベルで日本語化しました。
- **APIエラーハンドリング**: `GlobalExceptionHandler` を実装し、予期せぬエラーや権限エラーも含めて、統一された日本語メッセージを返却するようにしました。
- **フロントエンド**: 招待リンク無効時や通信エラー時のメッセージを、ユーザーに分かりやすい日本語に翻訳・統一しました。
