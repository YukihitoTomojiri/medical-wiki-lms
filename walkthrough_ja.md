...
- **ダッシュボード表示**: ユーザーのダッシュボード最上部に、重要度に応じたアイコン（赤・青・グレー）とともにお知らせが表示されます。
- **権限管理**: 管理者は自施設のお知らせのみ、開発者は全施設共通のお知らせを作成できるように制御されています。
- **サイドメニュー改善**: ADMINセクションに「お知らせ管理」メニューを追加し、アクティブ状態のハイライト判定を修正しました（完全一致・サブパス判定により、他のメニューを開いている際に別のメニューがハイライトされる問題を解消）。
- **投稿フォーム**: Material Design 3 に準拠したお知らせ作成フォームを実装しました。開発者は「全施設（共通）」か「自施設のみ」か配信対象を選択可能です。
