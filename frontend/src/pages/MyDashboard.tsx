        ...
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <button
                    ...
                    className={`text-left bg-white p-5 rounded-2xl border shadow-sm relative overflow-hidden group hover:shadow-md transition-all ${activeTab === 'notifications' ? 'border-sky-500 ring-2 ring-sky-100' : 'border-gray-100'
                        }`}
                >
                    <div className="absolute right-0 top-0 w-24 h-24 bg-sky-50 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110" />
                    <div className="relative">
                        <div className="flex items-center gap-2 text-gray-500 mb-2">
                            <Bell size={18} className="text-sky-500" />
                            <span className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'notifications' ? 'text-sky-600' : ''}`}>お知らせ</span>
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-3xl font-black text-gray-800">{dashboardData?.unreadNotificationsCount ?? 0}</span>
