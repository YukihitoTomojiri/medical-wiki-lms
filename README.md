...
### 💼 労務管理 (Paid Leave Management)
- **有給休暇申請**: 職員はサイドメニューの「有給申請」から希望日と理由を入力して申請が可能。
- **承認フロー**: 管理者および開発者は、管理者ダッシュボードの「労務管理」タブから申請一覧を確認し、承認/却下を選択できます。
- **代理操作**: `DEVELOPER` ロールのユーザーも管理者機能をフルに利用可能。操作中はヘッダーに「開発者権限で代行中」のバッジが表示され、誤認を防止します。
