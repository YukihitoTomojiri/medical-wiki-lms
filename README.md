...
### [Modern Header & Logo Design]
- **feat**: ヘッダーデザインを刷新し、ダークテーマ (`#2D1600`) とオレンジアクセントを採用。

### [Header Visibility Overhaul]
- **fix**: ヘッダーの視認性を抜本的に改善。
  - 背景色を `#2D1600` (ダークココア/ディープオレンジ) に変更し、白文字とのコントラスト比を最大化。
  - ロゴデザインを「白背景コンテナ＋オレンジアイコン」に変更し、どのような背景でも視認性を担保。
  - マイクロコピー（"想像力で〜"）を追加し、ブランドメッセージを強化。


- **不具合修正**: 開発者/管理者ユーザーでログイン時に管理画面へのリンクが消失していた問題を解決。
- **認可ロジック**: `useAuth` フックに `isAdmin` ヘルパーを追加し、フロントエンドでの権限判定を共通化。
- **M3 UI**: 管理メニューのアイコンと遷移をMaterial Design 3の Navigation Rail 仕様に合わせて最適化。
