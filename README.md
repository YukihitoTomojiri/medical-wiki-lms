...
#### レポート出力の仕様と文字コードについて
エクスポートされる各レポートは、以下の動作仕様に基づいて生成されます。

| レポート形式 | 文字コード | 仕様詳細 |
| :--- | :--- | :--- |
| **CSV** | **UTF-8 (BOM付き)** | 日本語を正常に表示させるため、UTF-8 BOMを採用しています。Excelでの直接参照でも文字化けが発生しません。 |
| **PDF** | **UTF-8 (フォント埋め込み)** | `Noto Sans JP` をファイル内に埋め込んでいます。閲覧環境のフォント有無に関わらず、正確なレイアウトと日本語表示が維持されます。 |

> [!NOTE]
> **フォントライセンスについて**: レポートで使用している `Noto Sans JP` は [SIL Open Font License 1.1](https://scripts.sil.org/OFL) に基づき配布されています。
