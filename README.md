...
- **統合開発者ダッシュボード (Integrated Developer Dashboard)**: 
  - システム統計、稼働ノード管理、操作履歴、および全ユーザー管理をタブ形式で一元管理。
  - ページ遷移時やメニュー選択時にデフォルトの「システム統計」へ自動遷移するナビゲーション制御を実装。
  - DEVELOPERロールによる厳格なアクセス制御。
  - *注*: ユーザー登録機能は「ユーザー管理」画面（`/admin/users`）へ移設されました。

### 👥 管理者ユーザー管理 (Admin User Management)
施設管理者（ADMIN）が自施設のユーザーを直接管理できる機能です。

- **アクセス権限**:
  - **DEVELOPER**: 全施設の全ユーザーを管理可能。
  - **ADMIN**: 自施設のユーザーのみ表示・編集・作成・削除が可能。他施設やDEVELOPERユーザーの操作は禁止。
- **機能一覧**:
  - **ユーザー一覧**: 施設ごとにフィルタリングされたユーザーリストを表示（DEVELOPERは全施設を切り替え可能）。
  - **ユーザー登録**: 職員番号、氏名、施設、部署、権限、入社日、有給日数を設定して新規登録。
  - **ユーザー編集**: 既存ユーザーの情報（施設変更はDEVELOPERのみ）を更新。
  - **ユーザー削除**: 論理削除により退職者をアーカイブ（復元は開発者ダッシュボードから可能）。
- **セキュリティ**:
  - ADMINは自施設の`USER`/`ADMIN`のみ操作可能、`DEVELOPER`ロールのユーザーは変更不可。
  - 施設を跨いだ操作はバックエンドでブロックされ、`AccessDeniedException`が発生。
