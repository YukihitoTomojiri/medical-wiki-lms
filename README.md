...
### 📅 今後の計画 (Roadmap)
- **高度なテストフェーズ**:
  - Playwrightによる主要業務シナリオのE2Eテスト自動化
  - k6を用いた同時アクセス時の負荷・パフォーマンス検証
  - CI/CDパイプラインへのセキュリティ脆弱性スキャン(DAST)の統合
  - WCAG 2.1準拠のアクセシビリティ自動チェックの導入
...
- **feat**: 研修削除フローを再調整。確認ダイアログを使用しつつ、削除成功時は画面上部の通知（トースト）を廃止し、一覧の即時更新のみを行う「静的な」フィードバックに変更。
- **fix**: 削除実行時の無限ループバグを修正。`loadData` 関数を `useCallback` でメモ化し、モーダルクローズとデータリロードの順序を最適化することで、再レンダリングループを防止。
- **fix**: 削除処理を完全に単純化。`loadData()` 呼び出しを廃止し、API実行後に直接 State を `filter()` で更新する方式に変更。これにより通知ループの根本原因を物理的に除去し、削除機能を完全に正常化。
- **refactor**: 研修削除機能を完全スクラッチ再実装。複雑化した通知ステートとカスタムモーダルを全廃し、ブラウザ標準の `window.confirm` と単純なState操作のみに一本化することで、点滅バグを根絶。
- **fix**: バックエンドの削除API (`DELETE /api/training/events/{id}`) におけるコンパイルエラー（メソッド未定義）を修正し、論理削除 (`deleted_at` 設定) を正しく実装。
- **fix**: 削除APIの認可設定を強化。`SecurityConfig` に `@EnableMethodSecurity` を追加し、Controller側で `@PreAuthorize("hasAnyRole('ADMIN', 'DEVELOPER')")` を明示することで、開発者（ROLE_DEVELOPER）が正しく削除を実行できるように修正。
...
### [権限マニュアル (Access Control)]
- **開発者 (ROLE_DEVELOPER)**:
  - システム全体の全データ（研修、お知らせ等）を施設制限なく閲覧・管理可能。
  - バックエンド診断ツールへのアクセス。
- **施設管理者 (ROLE_ADMIN)**:
  - 所属施設の研修イベントの作成・管理。
  - 所属施設ユーザーの学習状況・有給申請の管理。
  - **NEW**: 研修・お知らせの一覧は所属施設のデータに限定して表示。
- **一般ユーザー (ROLE_USER)**:
  - 自身が対象（委員会・職種）に含まれる研修の受講。
  - 自身のお知らせ閲覧。
  - 自身の有給休暇申請・履歴確認。
- **fix**: サイドメニューのアクティブ状態ハイライトを完全修正（`end`プロパティによる完全一致判定を導入し、ダッシュボードの誤ハイライトを解消）。
- **feat**: サイドメニューのADMINセクションに「お知らせ管理」メニューを追加。
- **feat**: お知らせ投稿フォームおよび一覧画面のUI刷新（`PageHeader`コンポーネント導入による統一感のあるデザイン、M3スタイル準拠）。

### [研修会・アンケート管理 (Training & Survey)] (feature/training-management)
- **feat**: 研修動画の視聴と理解度確認アンケートを統合した学習管理モジュール。
- **Targeted Training**: 職種や委員会指定による対象者限定の研修配信（例：「感染対策委員会」メンバーのみ）。
- **Multimedia**: YouTube動画埋め込みと資料（PDF等）の配布に対応。
- **QR Code Attendance**: 各研修に固有のQRコードを発行。集合研修での出席確認やスマホからのアンケート回答に活用可能。
- **Survey Analysis**: 受講後のアンケート回答を管理者ダッシュボードで一覧表示・一括コピー可能。
- **Triple Video Display**: 最大3つの動画を同時に表示可能な3カラムレイアウトに対応。多角的な学習をサポート。
- **Gemini Bulk Copy**: 生成AIによる分析を想定した回答データの一括コピー機能を実装。
- **Real-time Stats**: ダッシュボードの学習完了数がアンケート送信と同時にリアルタイムで更新。
- **Training Visibility Control**: ユーザーの所属拠点、職種、および委員会に基づく精密な研修表示ロジックを実装。
  - 拠点制限 (`facilityId`): 自身が所属する、または全施設向けの研修のみ表示。
  - 職種制限 (`targetJobType`): 自身の職種、または全職種向けの研修のみ表示。
  - 委員会制限 (`targetCommitteeId`): 自身が所属する委員会の研修のみ表示。
...
#### 最近の変更 (2026-02-12)
- **研修管理機能の改善**:
  - フォームリセット処理の修正: 新規作成・キャンセル時にフォームをクリアするよう改善。
  - エラー可視化: 研修会の作成・編集失敗時に詳細なエラーメッセージを表示。
  - 日付処理の堅牢化: `YYYY-MM-DD` 形式の日付文字列をバックエンドで柔軟にパースするよう修正。
  - YouTube URL対応: URL入力欄のUI改善とID抽出ロジックの強化、DBカラムの拡張(TEXT型)。
  - **研修詳細ページのUI改善**: 動画サムネイルを3カラムのグリッドレイアウトに変更し、サイドバーの日付表記を日本語化。13インチPCでも見やすく調整。
  - **コンテンツ配置の最適化**: 研修詳細ページで「目的・概要」を動画より上に配置し、ファーストビューで重要な情報が伝わるように改善。
...
#### 研修詳細ページの構成 (Training Detail)
- **Video Section**: YouTube URLを解析し、自動的にサムネイルを表示。再生ボタンをクリックするとインプレースで動画再生が開始されます。
- **Document Section**: 研修に関連する添付資料（PDF, Word, Excelなど）をカード形式で一覧表示し、クリックで閲覧・ダウンロードが可能。
- **Description Section**: 研修の目的や詳細な指示文をリッチなタイポグラフィで表示。
- **Questionnaire Section**: 研修受講後に「理解度（5段階）」「動画の見やすさ（5段階）」「自由記述コメント」を送信可能。回答送信と同時に出席が記録されます。
- **Access Control**: 自身がターゲット（拠点・委員会・職種）に含まれていない研修へのアクセスをエラー画面でブロックする厳格な権限管理を導入。
...
## ⚠️ トラブルシューティング：画面が白くなった時の対応

研修詳細ページなどで画面が真っ白（ホワイトアウト）になったり、動作が著しく重くなった場合の修正と対応方法です。

### 1. 修正の内容 (2026-02-12 安定化アップデート)
- **無限再レンダリングの防止**: フロントエンドの `useEffect` 指列を精査し、計算ロジックをメモ化 (`useMemo`) することで、ブラウザのメモリリークを解消。
- **DB接続の即時解放**: バックエンドの参照系処理に `@Transactional(readOnly = true)` を徹底し、接続プール不足によるハングアップを防止。
- **フォールバック実装**: 万が一のエラー発生時も、画面全体を落とさず安全なエラー文を表示するようにガードを強化。

### 2. 緊急時の対応
万が一、画面が復旧しない場合は、サーバーサイドの接続プールが枯渇している可能性があります。
以下の手順でシステムを再起動してください。
```bash
docker compose down && docker compose up -d
```
その後、ブラウザのキャッシュをクリアして再アクセスを試みてください。
