...
- **RBAC (Role Based Access Control)**:
  - **DEVELOPER**: システム全域の管理権限を持つスーパーユーザー。全施設のユーザーデータ、申請データ、システム統計を閲覧可能。
  - **ADMIN (施設管理者)**: **自身が所属する施設**のデータのみにアクセスを制限。管轄施設以外のユーザー情報の閲覧や、勤怠・有給申請の承認は行えません。
  - **USER**: 一般職員。自身のデータの閲覧と各種申請のみ。
- **データ同期**: 管理者が申請を承認・却下した際、ダッシュボードの「申請中」カウントやユーザーの有給残高が即座に同期されるよう強化。
...
- **有給休暇・入社日管理 (Advanced Leave Management)**: 
  - **法定付与の自動計算**: 日本の労働基準法に基づき、入社日からの勤続年数に応じて法定有給休暇を自動的に算出します。
  - **初期調整日数 (Carry-over)**: 既存職員の繰越日数や端数調整分を「初期調整日数」として設定可能です。
  - **動的な残高計算**: `残高 = 法定付与 + 初期調整 - 消化済み（承認済み申請）` の計算式に基づき、常に最新の残高を提示します。
  - **詳細な内訳表示**: 管理者およびユーザー自身のダッシュボードで、残高の算出根拠（法定分、調整分、消化済み、申請中）を詳細に表示します。
  - **整合性維持**: ユーザーの入社日や調整日数の変更は監査ログに記録され、統計データに即座に反映されます。
...
- **2026-02-07**: 有給休暇申請が管理者ダッシュボードに表示されない問題を修正。データベーススキーマとエンティティ定義の不整合（不要な `date` カラムの残存）により `INSERT` 処理が失敗していたため、マイグレーションスクリプトを追加してスキーマを修正しました。
- **2026-02-07**: 開発者ダッシュボードの申請数カードと一覧表示の件数が不整合となる問題を修正。
    - フロントエンド: 統計取得APIがハードコードされたIDを使用していたため、ログインユーザーのIDを正しく使用するように修正。
    - バックエンド: `PaidLeaveRepository` の全件取得メソッド (`findAllByOrderByStartDateDesc`) を明示的な JPQL クエリ (`JOIN FETCH` 含む) に変更し、データ取得の確実性を向上させました。これにより、関連するユーザー情報が確実にロードされ、一覧に表示されるようになります。
