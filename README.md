...
#### 2026-02-08: 【セキュリティ】管理者パスワードの強制リセットおよびパスワード変更強制機能の実装
- **課題**: 管理者アカウントのパスワード紛失によるログイン不能問題の解消。および、セキュリティ向上のための初回ログインパスワード変更の強制。
- **修正内容**:
  1. **Database**: 
     - `users` テーブルの `must_change_password` 属性を `NOT NULL DEFAULT 1` に変更。
     - `admin` ユーザーのパスワードを `admin123` にリセットし、変更強制フラグを `true` に設定。
  2. **Backend**: 
     - `AuthService.changePassword` に現在のパスワード（currentPassword）検証ロジックを追加。
     - `UserDto` を通じて `mustChangePassword` フラグを適切に伝播。
  3. **Frontend**: 
     - 日本語化された「パスワード変更画面（`ChangePassword.tsx`）」を実装。
     - ログインユーザーのフラグを確認し、変更が完了するまで他の全機能へのアクセスを遮断するルーティングガードを強化。
- **結果**: 🎉 **復旧および強化完了**
  - 管理者アカウントでの再ログインが可能となり、かつ強力な初回変更プロセスが導入されました。

#### 2026-02-08: 【UX改善・バリデーション】申請完了専用ページの実装と重複申請防止ロジックの導入
- **課題**: 
  - 申請後の通知が `alert()` のみで分かりにくかった。
  - ユーザーの誤操作による重複申請（同じ日に複数の有給や遅刻申請等）が発生し、管理側の負担となっていた。
- **修正内容**:
  1. **Frontend**: 
     - 申請完了専用画面（`SubmissionSuccessPage.tsx`）を新規作成。
     - `MyDashboard.tsx` における送信成功後の処理をリダイレクトに変更。
     - バリデーションエラー（重複等）をフォーム上に赤字で分かりやすく表示するよう改善。
  2. **Backend**: 
     - **有給休暇**: 既存の申請（却下以外）と1日でも期間が重なる場合、詳細なエラー（例：「〇月〇日から〇月〇日は既に申請済みです」）を返却。
     - **勤怠変更**: 同一ユーザー・同一日・同一タイプで PENDING/APPROVED の申請がある場合、重複エラーを返却。
- **結果**: 🎉 **操作の確実性とデータ整合性の向上**
  - 重複申請がシステム的にブロックされ、かつ申請完了が視覚的に強調されることで、スタッフが安心して利用できる環境を整えました。
