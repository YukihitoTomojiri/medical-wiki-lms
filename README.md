...
- **Status Pills**: 状態は「稼働中（翠）」「停止中（灰）」「警告（黄）」のシンプルなカラーピルで表示し、視認性を高めています。
- **Simplified List**: "IDENTITY NAME", "STATUS", "FACILITY" 等の項目を整理し、過度な装飾を排したフラットなリストデザインを採用しました。
- **Scrollable Container**: データ量が増加しても画面全体が縦伸びしないよう、リスト部分はコンテナ内でスクロール（Stickyヘッダー付き）するように設計されています。
- **Grouped Filtering**: "FILTERING" セクションに検索とフィルタを集約しています。
