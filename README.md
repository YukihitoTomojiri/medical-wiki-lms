...
1. **ステータス詳細の確認**:
   - 該当ノードの「Facility / Status」列を確認し、表示されている詳細メッセージ（例：「通信途絶」「リソース不足」「応答遅延」）を読み取ります。

2. **状況別対応**:
   - **通信途絶 (DOWN)**: ネットワーク接続を確認し、該当拠点のルーター再起動を指示してください。
   - **リソース不足 (WARNING)**: メモリ使用率が80%を超えています。不要なアプリケーションを終了するか、再起動を検討してください。
   - **DB接続エラー**: システム管理者に連絡し、データベースサーバーの稼働状況を確認してください。

3. **フィルタによる影響範囲の特定**:
   - 画面上部のフィルタで「WARNING」または「DOWN」を選択し、障害が特定の拠点のみか、全拠点で発生しているかを確認してください。全拠点の場合はセンターサーバー側の障害の可能性があります。
