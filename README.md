...
- **ロールベース・サイドバー (Role-based Sidebar)**:
  ...
  - **管理者 (Admin)**: 管理者ダッシュボード、ユーザー管理を追加表示。
  - **開発者 (Developer)**: 組織管理、開発者メニューを含む全項目を表示。
...
アプリケーション全体の一貫性を保つために、統一されたデザイントークンとユーティリティクラスを導入しました。

#### カラートークン（`tailwind.config.js`）

| トークン | 色コード | 用途 |
|---------|---------|------|
| `primary-600` | `#ea580c` | **メインアクセント**（ボタン、アクティブ状態、リンク） |
| `primary-50〜100` | オレンジ系淡色 | ホバー状態、背景アクセント |
| `primary-700〜800` | オレンジ系濃色 | ホバー時の濃い状態、グラデーション終点 |
| `success` | `#10b981` | 承認済み、正常系表示 |
| `warning` | `#f59e0b` | 注意喚起、申請中 |
| `danger` | `#ef4444` | エラー、却下、削除系 |
| `info` | `#3b82f6` | 情報表示 |

> `medical` トークンは後方互換のため `primary-600` にエイリアスされています。

#### 共通ユーティリティクラス（`index.css`）

| クラス名 | 説明 |
|---------|------|
| `.card` / `.card-hover` | 白背景・角丸・ボーダーの共通カード |
| `.btn-primary` | オレンジプライマリボタン |
| `.btn-secondary` | グレーセカンダリボタン |
| `.btn-danger` | 赤色デンジャーボタン |
| `.form-input` / `.form-label` | フォーム入力・ラベル |
...
- **ヘッダー**: `bg-white/80 backdrop-blur-xl`
