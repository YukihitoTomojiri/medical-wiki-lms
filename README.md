...
#### 2026-02-08: 管理者画面のUI刷新および一括承認機能の実装
- **課題**: 申請一覧が情報過多で読みにくく、また大量の申請を1件ずつ処理するのが非効率であった。
- **修正内容**:
  1. **UI改善 (2段表示)**: 
     - 「申請者」カラムに氏名と所属施設を併記。
     - 「希望日」カラムに日付と取得区分（全日/半休等）を併記。
     - ステータスバッジのデザインを刷新し、視認性を向上。
  2. **一括承認機能**:
     - 左端に複数選択用チェックボックスおよび「全選択」機能を追加。
     - 選択中のみ表示される「一括承認」ボタンを設置。
  3. **Backend API**:
     - `PaidLeaveController` および `AttendanceRequestController` に `bulk-approve` エンドポイントを新設。
- **結果**: 🎉 **成功**
  - 旧データと新データの両方に対して、一括で承認処理が行えることを確認。
  - プロフェッショナルで高密度な情報レイアウトを実現。
