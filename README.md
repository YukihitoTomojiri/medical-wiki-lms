...
- **v1.3.1 (Current)**: システムレビューに基づく改善（開発者権限の修正、研修対象表示の強化）。
- **v1.3**: 研修詳細画面のリッチUI化（YouTube/資料プレビュー）、開発者権限の研修参照修正。
- **v1.2**: ダッシュボードのインタラクティブ化（タブ切り替え）、開発者権限の修正。
- **feat**: ダッシュボード上部への「お知らせ」セクションに通知を統一（重複していた通知カードを廃止）。
- **feat**: 全施設共通または特定施設向けのお知らせ配信制御（一般ユーザーは自施設＋全施設のお知らせを合算して閲覧可能）。
