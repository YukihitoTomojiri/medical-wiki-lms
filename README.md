...
- **2026-02-07**: 有給休暇申請機能（PAID_LEAVE）を設計通りに再実装。
    - **Entity**: 論理削除 (`deleted_at`)、`status`, `startDate`, `endDate`, `reason` 等の基本フィールドを実装。
    - **Repository**: 論理削除済みデータを除外するクエリを実装。
    - **Security (RBAC)**: 閲覧権限を厳格化（USER:自分のみ、ADMIN:施設内のみ、DEVELOPER:全施設）。
    - **API**: 申請、履歴、承認/却下の各エンドポイントを整備。
