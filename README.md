...
- [feat] サイドメニューのアクティブ状態ハイライトを完全修正（`end`プロパティによる完全一致判定を導入し、ダッシュボードの誤ハイライトを解消）。
- [feat] サイドメニューのADMINセクションに「お知らせ管理」メニューを追加。
- [feat] お知らせ投稿フォームおよび一覧画面のUI刷新（`PageHeader`コンポーネント導入による統一感のあるデザイン、M3スタイル準拠）。

### [Myダッシュボード UI統合] (feature/announcement-management)
- **feat**: お知らせリストを独立した表示から、学習状況・有給休暇と並ぶ「サマリーカード」として統合。
...
- **refactor**: `Announcement` エンティティを直接返却する実装を廃止し、`AnnouncementDto` を導入してセキュアかつ軽量なデータ転送を実現。
